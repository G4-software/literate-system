<?php
    ...
    if(isset($_GET['issue_in_project_id']) && isset($_GET['team_id']))
    {
        ...
        $inputs = array('close_message' => array(   'label' => "Enter close message:",
                                                    'type' => "textarea",
                                                    'name' => "close_message",
                                                    'args' => "rows=10"),
        ...
        $page = array(  'title' => "Close issue #{$_GET['issue_in_project_id']}",
                        'page_title' => "Close issue #{$_GET['issue_in_project_id']}",
                        'user' => $user,
                        'blocks' => $blocks);
    ...
    if(count($_POST) == 3)
    {
        ...
        $db_query = $db->prepare("UPDATE `issues` SET `closed_by` = :user_id, `closed_on` = :closed_on, `close_message` = :close_message WHERE `issues`.`in_team` = :in_team AND `issues`.`issue_in_project_id` = :issue_in_project_id;");
